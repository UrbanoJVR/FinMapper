...
class TestTransactionRepositoryIT:
    ...
    def test_when_get_by_month_year_and_category_should_works(self,
                                                              save_transactions_from_different_months_and_categories):
    ...
    def test_when_get_last_uncategorized_and_no_uncategorized_transactions_should_return_none(self, db_test_it):
        result = self.sut.get_last_uncategorized()

        assert result is None

    def test_when_get_last_uncategorized_and_all_transactions_are_categorized_should_return_none(self, db_test_it):
        transaction = self.transaction_mother.random()
        self.category_repository.save(transaction.category)
        self.sut.save(transaction)
    ...
    def test_given_a_year_with_more_than_one_transactions_when_reqeust_years_with_transactions_then_return_one_year_list(
            self, db_test_it):
        transaction1 = Transaction(id=1, transaction_date=TransactionDate(date(2024, 12, 1)),
                                   amount=TransactionAmount(Decimal(100)),
                                   type=TransactionType.EXPENSE, category=None, concept="Concept 1")
