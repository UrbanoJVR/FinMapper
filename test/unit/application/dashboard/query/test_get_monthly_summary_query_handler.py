...
class TestGetMonthlySummaryQueryHandler(TestCase):
    ...
    def test_given_transactions_in_single_category_then_return_correct_summary(self):
        ...
        transaction1 = Transaction(transaction_date=TransactionDate(date(2024, 1, 15)), amount=TransactionAmount(Decimal("100.00")), concept="Mercadona", category=category)
        transaction2 = Transaction(transaction_date=TransactionDate(date(2024, 1, 20)), amount=TransactionAmount(Decimal("50.00")), concept="Carrefour", category=category)
    ...
    def test_given_transactions_in_multiple_categories_then_order_by_total_desc(self):
        ...
        transaction1 = Transaction(transaction_date=TransactionDate(date(2024, 1, 15)), amount=TransactionAmount(Decimal("100.00")), concept="Mercadona", category=category1)
        transaction2 = Transaction(transaction_date=TransactionDate(date(2024, 1, 16)), amount=TransactionAmount(Decimal("300.00")), concept="Alquiler", category=category3)
        transaction3 = Transaction(transaction_date=TransactionDate(date(2024, 1, 17)), amount=TransactionAmount(Decimal("50.00")), concept="Gasolina", category=category2)
    ...
    def test_given_uncategorized_transactions_then_group_as_sin_categoria(self):
        transaction1 = Transaction(transaction_date=TransactionDate(date(2024, 1, 15)), amount=TransactionAmount(Decimal("100.00")), concept="Sin categoría 1", category=None)
        transaction2 = Transaction(transaction_date=TransactionDate(date(2024, 1, 16)), amount=TransactionAmount(Decimal("50.00")), concept="Sin categoría 2", category=None)
    ...
    def test_given_multiple_transactions_per_category_then_sum_correctly(self):
        category = Category(id=1, name="Alimentación")
        transaction1 = Transaction(transaction_date=TransactionDate(date(2024, 1, 5)), amount=TransactionAmount(Decimal("25.50")), concept="Test 1", category=category)
        transaction2 = Transaction(transaction_date=TransactionDate(date(2024, 1, 10)), amount=TransactionAmount(Decimal("30.25")), concept="Test 2", category=category)
        transaction3 = Transaction(transaction_date=TransactionDate(date(2024, 1, 15)), amount=TransactionAmount(Decimal("44.25")), concept="Test 3", category=category)
